...
number_of_folds = 4     # число сгибов
cv_qualification_model = sk_linear_models.LogisticRegression()   # cross validation qualification model
...
model_qualification_probability = qualification_model.predict_proba(X)[:, 1]     # предсказанные данные из модели
qualifies_double_grade["modeled_probability"] = model_qualification_probability
...
plt.clf()   # чистом старые графики для отрисовки ROC кривой
plt.xlabel("False positive rate")
plt.ylabel("True positive rate")
false_positive_rate, true_positive_rate, thresholds = sk_metrics.roc_curve(y, model_qualification_probability)
plt.plot(false_positive_rate, true_positive_rate)

roc_auc = sk_metrics.roc_auc_score(y, model_qualification_probability)
print(f"Area under curve: {roc_auc}")   # auc - area under curve
