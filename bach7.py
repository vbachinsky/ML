...
print('MSE = {}'.format(get_MSE(linear_regression, X_train, y_train)))
...
for z in range(len(labels) - 1):
    ...
    labels_2_futures = np.delete(labels, z)
    model_2_futures = train_linear_model(X_train_2_features, y_train)
    print(model_to_string(model_2_futures, labels_2_futures))
    print('Test MSE = {} \n'.format(get_MSE(model_2_futures, X_test_2_features, y_test)))
